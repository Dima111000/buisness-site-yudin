 ...
 */
    ...
    (() => {
        ...
        const inputMasks = document.querySelectorAll('input[name="number"]');
        ...
        const currentYearBlock = document.getElementById("currentYear");
        if (currentYearBlock) currentYearBlock.textContent = currentYear;
        ...
        function windowLoad() {
            function digitsCountersInit(digitsCountersItems) {
                ...
                if (digitsCounters) digitsCounters.forEach((digitsCounters => {
                    digitsCountersAnimate(digitsCounters);
                }));
            }
            function digitsCountersAnimate(digitsCounter) {
                let startTimestamp = null;
                const duration = parseFloat(digitsCounter.dataset.digitsCounters) ? parseFloat(digitsCounter.dataset.digitsCounters) : 1350;
                const startValue = parseFloat(digitsCounter.innerHTML);
                const startPosition = 0;
                const step = timestamp => {
                    if (!startTimestamp) startTimestamp = timestamp;
                    const progress = Math.min((timestamp - startTimestamp) / duration, 1);
                    digitsCounter.innerHTML = Math.round(progress * (startPosition + startValue));
        ...
        const coachesCards = document.querySelectorAll(".meet-our-teem-card-block__card");
        if (coachesCards.length) coachesCards.forEach((card => {
